   ...
   async getById(userID){
    try {
       const user= await UserModel.findOne(
        {_id:userID}).select('-tokens -_id -password -__v');  // user with selected fields
        return user;
    } catch (error) {
...
try {
   ...
   async addTokenInDb(user,token){
    ...
    if(oldTokens.length){  //oldtokens found
        oldTokens=oldTokens.filter(t=>{
            const timeDiffInSec=(Date.now()-parseInt(t.signedAt))/1000;//time diff of token 
            if(timeDiffInSec<14400){ //14400 seconds or 4 hour
                return t;
            }
        })
    }
   ...
   return jwtBlacklist.add({token:tokeninfo.token,signedAt:tokeninfo.signedAt});
  }
